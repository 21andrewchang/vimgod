<script lang="ts">
	import { signInWithGoogle } from '$lib/stores/auth';
	import Footer from '$lib/components/Footer.svelte';
	import BgDarkTiles from '$lib/components/BgDarkTiles.svelte';
</script>

<svelte:head>
	<title>Login - vimgod</title>
	<meta name="description" content="Sign in to vimgod - ranked vim. gg." />
</svelte:head>

<BgDarkTiles />

<div class="w-full relative overflow-hidden min-h-screen">
	<div class="relative z-[2] px-6 py-8 max-w-4xl mx-auto min-h-screen flex flex-col">
		<!-- Login Section -->
		<div class="flex flex-col items-center justify-center flex-1 space-y-8">
			<div class="text-center space-y-4">
				<p 
					class="text-lg"
					style="color:#c9ced6; font-family: 'JetBrains Mono','Fira Code',ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',Monaco,monospace;"
				>
					sign in to track your progress and compete on the leaderboard
				</p>
			</div>
			
			<!-- Google Sign In Button -->
			<button
				onclick={signInWithGoogle}
				class="group relative flex items-center gap-3 px-6 py-3 bg-white text-gray-900 rounded-lg font-medium transition-all duration-200 hover:bg-gray-100 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-black"
				style="font-family: 'JetBrains Mono','Fira Code',ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',Monaco,monospace;"
			>
				<!-- Google Icon -->
				<svg width="20" height="20" viewBox="0 0 24 24" class="flex-shrink-0">
					<path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
					<path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
					<path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
					<path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
				</svg>
				
				<!-- Button Text -->
				<span>continue with google</span>
				
				<!-- Hover Arrow -->
				<svg 
					width="16" 
					height="16" 
					viewBox="0 0 24 24" 
					fill="none" 
					stroke="currentColor" 
					stroke-width="2" 
					stroke-linecap="round" 
					stroke-linejoin="round"
					class="transition-transform duration-200 group-hover:translate-x-1"
				>
					<path d="M5 12h14M12 5l7 7-7 7"/>
				</svg>
			</button>
			
			<!-- Additional Info -->
			<div class="text-center space-y-2">
				<p 
					class="text-sm"
					style="color:#a8b0b8; font-family: 'JetBrains Mono','Fira Code',ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',Monaco,monospace;"
				>
					by signing in, you agree to our terms of service
				</p>
				<p 
					class="text-xs"
					style="color:#8a8a8a; font-family: 'JetBrains Mono','Fira Code',ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',Monaco,monospace;"
				>
					your progress will be saved and synced across devices
				</p>
			</div>
		</div>
	</div>
</div>

<Footer />
