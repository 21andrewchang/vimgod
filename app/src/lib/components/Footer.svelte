...
<footer bind:this={footerElement} class="fixed bottom-0 left-0 right-0 z-[2] bg-transparent">
            ...
            <div class="flex items-center gap-6">
                {#each links as link}
                                {#if link.name === 'twitter'}
                                    <div 
                                        class="flex items-center gap-2 text-sm transition-colors duration-200 {(isTwitterTextHovered || isBubbleHovered) ? 'text-white' : ''} cursor-pointer animated-underline {(isTwitterTextHovered || isBubbleHovered) ? 'twitter-hovered' : ''}"
                                        ...
                                        on:mouseenter={handleTwitterMouseEnter}
                                        on:mouseleave={handleTwitterMouseLeave}
                                    >
                                        <svg 
                                            width="16" 
                                            height="16" 
                    ...
                    {:else}
                        <a 
                            ...
                            class="flex items-center gap-2 text-sm transition-colors duration-200 hover:text-white animated-underline"
                            style="color:#c9ced6; font-family: 'JetBrains Mono','Fira Code',ui-monospace,SFMono-Regular,Menlo,Consolas,'Liberation Mono',Monaco,monospace;"
                        >
                            <svg 
                                width="16" 
    ...
    {#if twitterTooltip.show}
        <div 
            class="absolute z-[9999] px-4 py-3 rounded-md shadow-lg twitter-bubble {twitterTooltip.hiding ? 'hiding' : ''}"
            style="
