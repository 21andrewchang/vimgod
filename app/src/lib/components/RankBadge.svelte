<script lang="ts">
	...
	import Supernova from './Supernova.svelte';
	import Singularity from './Singularity.svelte';

	const { rank, rankName, elo, lp } = $props<{
		rank?: string;
		rankName?: string;
		elo?: number;
		lp?: number;
...
<style>
	:global(.bg-pearlescent) {
        ...
        /* soft radial highlight */
			...
			/* conic layer A (blue/violet) — off-center */
				conic-gradient(
					from 30deg at 28% 42%,
					rgba(231, 246, 255, 0.7) 0deg,
					rgba(230, 233, 255, 0.7) 90deg,
					rgba(240, 232, 255, 0.7) 180deg,
					rgba(233, 238, 255, 0.7) 270deg,
					rgba(231, 246, 255, 0.7) 360deg
				),
			...
			/* neutral base */ linear-gradient(120deg, #f7f9ff 0%, #edf1fa 100%);
	...
	:global(.shiny-glow) {
		...
		isolation: isolate;
		box-shadow:
			0 0 8px hsla(220, 100%, 80%, 0.3),
			0 0 18px hsla(230, 100%, 78%, 0.24),
			0 0 36px hsla(210, 100%, 70%, 0.2);
	}
